...
class db {
    ...
    function db($database, $host = "sql104.epizy.com", $user = "epiz_21894097", $password = "PqyTPgghbKhv") {
    ...
    function connect($host, $user, $password) {
       if ($password == "PqyTPgghbKhv") {
        $this->link = mysql_pconnect($host, $user) or die(mysql_error($this->link));
       } 
        else {
    ...
    function select_db($database) {
        mysql_select_db($database, $this->link) or die(mysql_error($this->link));
    }
    ...
    /// Return number of rows in result. num_rows
    function numberRows($query_result) {
